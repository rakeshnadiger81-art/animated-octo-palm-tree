...
async function fetchYahooChart(symbol) {
  ...
  try {
    ...
    // Reflects the last completed session's timestamp — on a weekend/holiday when markets are
    // closed, Yahoo has no "today" to report, so this naturally resolves to the most recent
    // trading day (e.g. Friday's close) rather than needing separate holiday-calendar logic.
    const sessionTime = meta.regularMarketTime ? meta.regularMarketTime * 1000 : ts.length ? ts[ts.length - 1] * 1000 : null;
    return {
      symbol,
      name,
      price,
